        ...
        <div class="column">
            ...
            <div class="ui column grid">
                <div class="center aligned column">
                    <p>
                        Got an account? <router-link to="/login">Login In</router-link>
                    </p>
                </div>
            </div>
        </div>
...
export default {
  ...
  /*
    Using Vue router's beforeRouteEnter navigation guard, we check if a knocle-token is available in
    localStorage which indicates the user is already logged in. If present, we simply redirect to the
    user homepage, and if not, we show them the signup form.
    */
    ...
    signup () {
      axios
        ...
        .then(response => {
          ...
          localStorage.setItem('knocle-token', response.data.data.token)
